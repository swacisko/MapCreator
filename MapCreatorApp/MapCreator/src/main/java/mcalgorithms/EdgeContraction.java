...
public class EdgeContraction {
    ...
    /**
     * This function checks, whether two neighbours of vertex n have other common neighbour or are connected in graph by an edge.
     * @param n
     * @return true, if these neighours are connected or have other common neighbour, false otherwise
     */
    private boolean commonNeighbours(MapNode n){
        MapNode n1 = n.getNeighbour(0);
        MapNode n2 = n.getNeighbour(1);
        if( graph.getMapEdgeWithNeighbours( n1.getID(), n2.getID() ) != null ) return true;
        for( MapNode node : graph.getNodes() ){
            if( node.getID() != n.getID() && n1.hasNeighbourOfId( node.getID() ) && n2.hasNeighbourOfId(node.getID()) ){
                return true;
            }
        }
    ...
    private void createDeg2Vertices() {
        ...
        for (MapNode n : graph.getNodes()) {
            if ((n.countEdges() == 2) && n.isContractable()/* && commonNeighbours(n) == false*/ ) {                
                deg2Nodes.add(n);
            }
        }
        System.out.println();
    ...
    /**
     * It may happen, that between two nodes there are multiple edges. I remove
     * one of them, assigning the edges contents to the others data. I remove
     * and edge only if there are exactly two parallel edges
     */
